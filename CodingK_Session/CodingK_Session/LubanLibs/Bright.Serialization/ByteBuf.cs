...
#pragma warning restore CA1819 // 属性不应返回数组
        ...
        private EDeserializeError TryReadSegment(int maxSize, out int segReadIndex, out int segWriteIndex, out int nextSegReadIndex)
        {
            segReadIndex = ReaderIndex + 1;
            segWriteIndex = nextSegReadIndex = 0;

            int tempReaderIndex = ReaderIndex;

            int n;
            int h = Bytes[tempReaderIndex];
            if (h < 0x80)
            {
                n = h;
                tempReaderIndex++;
            ...
            {
                if (!CanRead(2)) { return EDeserializeError.NOT_ENOUGH; }
                n = ((h & 0x3f) << 8) | Bytes[tempReaderIndex + 1];
                tempReaderIndex += 2;
            }
            else if (h < 0xe0)
            {
                if (!CanRead(3)) { return EDeserializeError.NOT_ENOUGH; }
                n = ((h & 0x1f) << 16) | (Bytes[tempReaderIndex + 1] << 8) | Bytes[tempReaderIndex + 2];
                tempReaderIndex += 3;
            }
            else if (h < 0xf0)
            {
                if (!CanRead(4)) { return EDeserializeError.NOT_ENOUGH; }
                n = ((h & 0x0f) << 24) | (Bytes[tempReaderIndex + 1] << 16) | (Bytes[tempReaderIndex + 2] << 8) | Bytes[tempReaderIndex + 3];
                tempReaderIndex += 4;
            }
            else
            {
            ...
            segWriteIndex = segReadIndex + n;
            nextSegReadIndex = tempReaderIndex + n;

            if (nextSegReadIndex > WriterIndex)
            {
                return EDeserializeError.NOT_ENOUGH;
            }


            if (tempReaderIndex > ReaderIndex + 1)
            {
                for (int i = ReaderIndex + 1; i < tempReaderIndex; i++)
                {
                    Bytes[i] = Bytes[i + n];
                }
            }
        ...
        public EDeserializeError TryEnterSegment(int maxSize, out SegmentSaveState saveState)
        {
            var err = TryReadSegment(maxSize, out int segReadIndex, out int segWriteIndex, out int nextSegReadIndex);
            if (err == EDeserializeError.OK)
            {
                saveState = new SegmentSaveState(nextSegReadIndex, WriterIndex);
                ReaderIndex = segReadIndex;
                WriterIndex = segWriteIndex;
                return EDeserializeError.OK;
            }
            saveState = default;
            return err;
